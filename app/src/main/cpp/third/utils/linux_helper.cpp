...
int MapsHelper::refresh(const string &libPath, const string &wantPerm) {
    if (!get_process_maps(libPath, wantPerm)) {
        return -1;
    }
...
void *MapsHelper::get_module_base(const string &libPath) {
    regex regLibPath(libPath);
    void *start = (void *) -1;
    for (const auto &item: mapsInfo) {
        if (!regex_match(item.path, regLibPath)) {
            continue;
        }
...
bool MapsHelper::get_process_maps(const string &libPath, const string &wantPerm) {
    mapsInfo.clear();
    regex regLibPath(libPath);
    ...
    while (!feof(fp)) {
        ...
        if ((!libPath.empty() && !regex_match(path, regLibPath)) ||
